 ...
 * folderInput omitted defaults to "general" — matched case-insensitively
 * against existing subfolders first (so it lands in the account's real
 * "General" folder rather than creating a lowercase near-duplicate, and same
 * for any other name, e.g. "restaurants" → "Restaurants"), falling back to
 * creating a new subfolder with the given name if nothing matches — Dynamic
 * Folders auto-create on first upload into them. Always a real subfolder
 * under GALLERY_ROOT, never the bare root itself. */
export async function uploadImageBuffer(
  ...
  let folderName = folderInput?.trim() || 'general';
  try {
    ...
    // Best-effort normalization — fall through with the given/default casing if this lookup fails.
  }
  ...
  return { url: json.secure_url, id: json.public_id, folder: folderName };
}
